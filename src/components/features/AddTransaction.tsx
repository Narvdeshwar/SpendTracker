...
export const AddTransaction: React.FC<AddTransactionProps> = ({ 
  onBack, onSave, accounts, friends, onAddFriend, initialCategory, editData 
}) => {
  ...
  const [accountId, setAccountId] = useState(editData?.account_id || accounts[0]?.id || '');
  const [selectedFriends, setSelectedFriends] = useState<string[]>(
    editData?.splits?.map(s => s.friend_id) || []
  );
  ...
  /**
   * We have replaced the custom manual keypad with a system-integrated 
   * numeric input. This ensures that the OS keyboard (Android/iOS) 
   * is used for entering amounts, as requested.
   */
  ...
  return (
    <div className="flex-1 flex flex-col bg-bg h-full">
      ...
      <div className="flex-1 flex flex-col px-8 pt-8 space-y-8 overflow-y-auto no-scrollbar pb-32">
        {/* Amount Display with System Keyboard Support */}

...
import { ChevronLeft, User, FileDown, ChevronRight, SlidersHorizontal, Zap, LogOut, FileUp, Check, Edit2 } from 'lucide-react';
...
export const UserManagement: React.FC<UserManagementProps> = ({ 
  onBack, onExport, onImport, session, updateProfile 
}) => {
  const fileInputRef = React.useRef<HTMLInputElement>(null);
  const [isEditing, setIsEditing] = useState(false);
  const [name, setName] = useState(session?.user?.user_metadata?.full_name || 'User');
  ...
  return (
    <div className="flex-1 flex flex-col bg-bg h-full">
      ...
      <div className="px-8 pt-12 space-y-10 flex-1 overflow-y-auto no-scrollbar pb-10">
        <div className="flex flex-col items-center gap-4">
          <div className="w-24 h-24 rounded-4xl bg-purple-600/10 flex items-center justify-center border-2 border-purple-600/20 shadow-xl shadow-purple-600/5 transition-transform active:scale-95">
          ...
          <div className="text-center w-full max-w-[240px] space-y-2">
            {!isEditing ? (
              <div className="flex items-center justify-center gap-2 group cursor-pointer" onClick={() => setIsEditing(true)}>
                ...
                <Edit2 size={14} className="text-purple-600 opacity-0 group-hover:opacity-100 transition-opacity" />
              </div>
            ) : (
              <div className="flex gap-2">
                <input 
                  ...
                  className="w-full glass p-3 rounded-xl text-center font-bold focus:outline-none focus:ring-2 focus:ring-purple-600/20"
                />
                <button onClick={handleSaveProfile} className="bg-purple-600 text-white p-3 rounded-xl shadow-lg shadow-purple-600/20"><Check size={18} /></button>
              </div>
            )}
            <p className="text-[10px] font-bold text-ink opacity-30 mt-1 lowercase select-all">{session?.user?.email}</p>
